...
from config.sqlite_health import sqlite_file_path_from_uri
...
class DatabaseConfig:
    ...
    def _build_sqlite_url(self):
        ...
        if sqlite_path:
            if os.path.isabs(sqlite_path):
                ...
                full_path = (base_path / sqlite_path).resolve()
                return f'sqlite:///{full_path}'

        # Default SQLite path — project root (always absolute)
        if self.base_dir:
            base_path = Path(self.base_dir) if isinstance(self.base_dir, str) else self.base_dir
            db_path = (base_path / 'armillarylab.db').resolve()
        else:
            db_path = Path('armillarylab.db').resolve()
    ...
    def _project_is_onedrive_synced(self) -> bool:
        base_path = Path(self.base_dir) if self.base_dir else Path(".")
        return os.name == "nt" and "onedrive" in str(base_path.resolve()).lower()

    def _sqlite_wal_enabled(self) -> bool:
        """WAL sidecar files (.db-wal/.db-shm) often break SQLite on OneDrive."""
        if self._project_is_onedrive_synced():
            return os.getenv("SQLITE_WAL_MODE", "false").lower() == "true"
        return os.getenv("SQLITE_WAL_MODE", "true").lower() == "true"
    ...
    def get_engine_args(self):
        ...
        if self.db_type == 'postgresql':
            ...
            # SQLite: NullPool avoids stale pooled connections across Flask reloads
    ...
    def sqlite_file_path(self) -> Path | None:
        if self.db_type != 'sqlite':
            return None
        return sqlite_file_path_from_uri(self.connection_string)

    def sqlite_connect_pragmas(self):
        """PRAGMAs applied on each SQLite connection."""
        if self.db_type != 'sqlite':
            return []
        pragmas = [
            ("foreign_keys", "ON"),
        ]
        if not self._sqlite_wal_enabled():
            pragmas.extend([
                ("journal_mode", "DELETE"),
                ("synchronous", "FULL"),
            ])
